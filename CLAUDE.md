...
```bash
deploy.sh [-ssh] <target> [cnf]
```

- `-ssh`: 省略可。デプロイ（pull/post_pull）を行わず、`.deploy` の `host`/`user`/`identity_file`/`password` を使って対象へインタラクティブにssh接続するだけのモード（sshコマンドのエイリアス代わり）。`path` が設定されていればそこに `cd` してからログインシェルを起動する
- `target`: `.deploy` 内のセクション名（例: `45`, `prod`）
- `cnf`: 設定ファイルパス。省略時はカレントディレクトリの `./.deploy`
- 呼び出し元のカレントディレクトリ＝デプロイ対象プロジェクトのローカル作業ディレクトリを想定（未pushコミットのチェックに使う。`-ssh` モードではこのチェックは行わない）
...
- `pre_pull`: 省略可。ssh接続を張る前にローカル（呼び出し元の作業ディレクトリ）で実行する任意コマンド。`public/build`のような、gitやsvnの管理下に置いていないビルド成果物をrsync等でリモートへ転送する用途を想定。非ゼロ終了で即座にデプロイを中断し、ssh接続自体を行わない（コードだけ先にpullされて資産とズレる事故を防ぐため）。post_pullと同様スクリプト側で特別扱いしているコマンドはなく、rsyncの接続先・認証情報はこのキーの中に自分で書く（`host`/`password`キーとは別物。多少の重複は許容する）
...
- `log`: 省略可（既定`false`＝出力しない）。`true`でリモート側 `path` 直下の `deploy.log` に追記、パスを指定するとそこに追記する（相対パスは `path` 基準。ディレクトリが無ければ自動作成）。pull/post_pullの標準出力・標準エラー出力を、ターミナル表示はそのままにログファイルにも書き込む（`tee`）
...
- **ログ出力は任意でリモート側に`tee`追記**: `log`キーを設定した場合のみ、リモートコマンドの先頭で`exec > >(tee -a "<ログパス>") 2>&1`を実行し、以降のpull/post_pullの出力をターミナル表示と同時にログファイルへ追記する。`log`未設定時（既定）はこの行自体を組み込まず、従来通りログ出力は一切行わない。この`exec > >(...)`はbash専用構文のため、**リモートのログインシェルがbashであることが前提**（大半のLinuxサーバでは満たされる想定。dash等の場合は構文エラーになる）。ログファイルの親ディレクトリは`mkdir -p`で自動作成する。
- **pre_pullはssh接続前にローカルで実行**: `public/build`のようなVCS管理外のビルド成果物をrsync等でリモートへ転送する用途を想定した、ssh接続前のローカルフック。post_pullと対称の「任意コマンドを`bash -c`で実行するだけ」の薄いフックで、rsyncの接続解決やオプション（`--delete`等）の意味論はdeploy.sh側には持たせず、コマンド文字列として`.deploy`側に自由記述してもらう。非ゼロ終了時はその場でssh接続を行わずに中断する（実行順を「pre_pull→ssh接続」に固定しているのは、逆順だと新しいコードだけ先に反映されて古い資産のままの瞬間ができてしまうため）。
- **汎用rsyncラッパー化はスコープ外**: `pre_pull`はあくまで「任意コマンドの実行」に留め、`sync_dir`のような専用キーでrsyncの接続先解決や`--delete`等の意味論をdeploy.sh本体に持たせることはしない（案件ごとに転送要件が異なり、汎用ツールへの結合が強くなりすぎるため）。
- **`-ssh`はssh接続のみの薄いエイリアス**: pull/post_pull/pre_pull/未push警告/logは一切行わず、`.deploy`の接続情報（host/user/identity_file/password）を使って`ssh -t`するだけ。`path`が設定されていればリモートで`cd "$path" && exec ${SHELL:-bash} -l`してからログインシェルへ入る（未設定ならそのままログイン）。パスワード認証時のsshpass経由・ホスト鍵自動登録・失敗時の診断メッセージはデプロイモードと共通のロジックを使う。
