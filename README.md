...
デプロイは行わず`.deploy`の接続情報でsshログインだけしたい場合は`-ssh`を付ける（後述）。

```bash
deploy -ssh <target> [cnf]
```
...
デプロイは行わず、`.deploy`の接続情報（`host`/`user`/`identity_file`/`password`）でそのままsshログインしたいだけの場合は`-ssh`を付ける。sshコマンドのエイリアス代わりに使える。

```bash
deploy -ssh <target> [cnf]
```

- `pull`/`post_pull`/`pre_pull`は一切実行しない。未pushコミットの警告や`log`によるログ出力も行わない
- `path`が設定されていれば、接続後そこに`cd`してからログインシェルを起動する。未設定ならそのまま通常のログインになる
- パスワード認証（`sshpass`経由）やホスト鍵の初回自動登録、失敗時の診断メッセージはデプロイ時と共通のロジックを使う
...
- **`-ssh`はssh接続のみの薄いエイリアス**: pull/post_pull/pre_pull/未push警告/logは行わず、`.deploy`の接続情報でsshログインするだけ（詳細は前述）
